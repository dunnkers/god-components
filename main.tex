...
%Some components like tika.batch, tika.example, tika.parser, tika.mime, parser.microsoft, parser.txt and tika.sax have the highest amount of developers that contributed, approximately 120 developers for each package. Other components like tika.fork and tika.server have a significantly smaller amount of developers involved, 52 developers and 47 developers, respectively. Some of the God components that had the least amount of developers are tika.utils, .parser.microsoft.onenote, .microsoft.chm, with 8 developers, 26 developers and 14 developers, respectively.
...
%In order to obtain a better overview of the God components, we decided to perform an analysis mainly on the issues that had the most significant impact in evolution of the God components. Based on this criteria, we chose the issues that added or removed the highest amount of classes. 

%This section focuses on analyzing a set of issues that added and removed the highest amount of classes within the respective God components. 

%This section focuses on analysing a set of issues that had the most noticeable impact in adding and removing classes from the God components. After that, we analyze these issues in order to determine the possible rationale of the developers when it comes to adding or removing these God components. The analysis is performed based on the following questions:
\subsection{Issues}
