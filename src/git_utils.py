...
def compute_locs(godcomps, commit_id):
    ...
    locdf = get_commit_locs(commit_id)
    all_files = locdf['file'].copy()
    for package in godcompsdf['package'].values:
        # find files that affected this GC
        path = 'src/main/java/{}'.format(package.replace('.', '/'))
        ...
        total['commit'] = commit_id
        results.append(total)
    new_df = pd.DataFrame(results)
    return new_df

def get_locs(all_commits, all_reports):
    ...
    mapped_commits = []
    for _, commit in tqdm(all_commits.iterrows(), total=all_commits.shape[0]):
        mapped_commits.append(compute_locs(godcomps, commit.commit))
